-- Studio Approval Workflow Migration
-- Adds approval status tracking and admin oversight for studio onboarding
-- Date: 2025-11-10

-- ============================================
-- PART 1: ADD APPROVAL FIELDS TO STUDIOS TABLE
-- ============================================
...
CREATE TABLE IF NOT EXISTS studio_onboarding_submissions (
    ...
    tax_id TEXT,
    business_type TEXT CHECK (business_type IN ('studio', 'independent_instructor', 'venue')),
...
CREATE INDEX IF NOT EXISTS idx_submissions_user_id ON studio_onboarding_submissions(user_id);
CREATE INDEX IF NOT EXISTS idx_submissions_studio_id ON studio_onboarding_submissions(studio_id);
CREATE INDEX IF NOT EXISTS idx_submissions_status ON studio_onboarding_submissions(submission_status);
...
CREATE POLICY "Admins can view all studios" ON studios
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM user_profiles
            WHERE id = auth.uid()
            AND (profile->>'role' = 'admin' OR profile->>'role' = 'super_admin')
        )
    );
...
CREATE POLICY "Admins can view all submissions" ON studio_onboarding_submissions
    FOR SELECT
    USING (
        EXISTS (
            SELECT 1 FROM user_profiles
...
CREATE POLICY "Admins can update all submissions" ON studio_onboarding_submissions
    FOR UPDATE
    USING (
        EXISTS (
            SELECT 1 FROM user_profiles
