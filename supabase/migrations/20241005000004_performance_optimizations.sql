...
-- Note: venues table doesn't exist in current schema (using studios table instead)
...
-- Note: instructors table exists but may not have user_id column, using simple public read policy
CREATE POLICY "Instructors are publicly viewable_optimized" 
    ON instructors 
    FOR SELECT 
    USING (is_active = true);

CREATE POLICY "Service role can manage instructors_optimized" 
    ON instructors 
    FOR ALL 
    USING (auth.role() = 'service_role')
...
-- Note: instructors table doesn't have user_id column
...
COMMENT ON POLICY "Instructors are publicly viewable_optimized" ON instructors IS 
'Simplified instructor policy for public access';

-- Note: venues table doesn't exist in current schema
