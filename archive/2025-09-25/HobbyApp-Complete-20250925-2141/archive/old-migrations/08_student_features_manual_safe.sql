-- Migration 08: Student Features (Manual Deploy - Safe Version)
...
BEGIN
    ...
    LOOP
        IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = table_name) THEN
            RAISE NOTICE 'Migration 08 verification: % table exists ✓', table_name;
        ELSE
            RAISE WARNING 'Migration 08 issue: % table missing!', table_name;
        END IF;
    END LOOP;
END $$;